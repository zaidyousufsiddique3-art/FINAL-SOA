...
export const ManualEntryForm: React.FC<ManualEntryFormProps> = ({
  ...
  const inputClass = "w-full bg-gray-50 dark:bg-[#0B0C10] border border-gray-200 dark:border-[#2A2D33] text-gray-900 dark:text-white rounded p-2 text-sm outline-none focus:border-[#5B6CFF] focus:ring-1 focus:ring-[#5B6CFF] transition-all placeholder-gray-400 dark:placeholder-gray-600";
  const labelClass = "text-[10px] font-bold text-gray-500 dark:text-[#A1A5B0] mb-1 uppercase tracking-wider";
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        <h3 className="font-bold text-gray-500 dark:text-[#A1A5B0] flex items-center gap-2 text-sm border-b border-gray-200 dark:border-[#2A2D33] pb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="flex flex-col">
            ...
            >
              ...
              {customers.map(c => <option key={c} value={c} className="bg-white dark:bg-[#16181D]">{c}</option>)}
            </select>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <input type="text" value="PAYMENT" disabled className="w-full bg-gray-100 dark:bg-[#16181D] border border-gray-200 dark:border-[#2A2D33] text-gray-400 dark:text-gray-400 rounded p-2 text-sm cursor-not-allowed" />
          </div>
        ...
        <div className="flex items-center gap-4 pt-2">
          <button type="submit" className="bg-gray-100 dark:bg-[#2A2D33] text-[#5B6CFF] border border-[#5B6CFF] px-6 py-2 rounded-lg text-sm font-bold hover:bg-[#5B6CFF] hover:text-white transition-all shadow-lg flex items-center gap-2">
            <PlusCircle size={16} />
            Add Payment to List
          </button>

          {showSuccess && (
            <div className="flex items-center gap-2 text-green-500 dark:text-green-400 animate-in fade-in slide-in-from-left-2 duration-300">
              <CheckCircle size={16} />
      ...
      {customerTransactions.length > 0 && (
        <div className="mt-6 border border-gray-200 dark:border-[#2A2D33] rounded-lg overflow-hidden bg-gray-50 dark:bg-[#0B0C10]">
          <div className="bg-gray-100 dark:bg-[#16181D] px-4 py-2 border-b border-gray-200 dark:border-[#2A2D33] flex items-center gap-2">
            <List size={14} className="text-[#14F1E1]" />
            <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Added Payments ({customerTransactions.length})</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-gray-500 dark:text-gray-400">
              <thead className="bg-white dark:bg-[#16181D] text-gray-700 dark:text-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-[#2A2D33]">
                {customerTransactions.map((trx) => (
                  <tr key={trx.id} className="hover:bg-gray-100 dark:hover:bg-[#1C1F24]">
                    ...
                    <td className="p-3 text-right text-red-500 dark:text-red-400">({formatCurrency(Math.abs(trx.originalAmount))})</td>
