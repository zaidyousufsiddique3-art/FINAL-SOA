...
export const FileUpload: React.FC<FileUploadProps> = ({ onDataLoaded }) => {
  ...
  let containerClasses = "border border-dashed rounded-lg px-4 py-3 flex items-center justify-center transition-all cursor-pointer group bg-gray-50 dark:bg-[#0B0C10] shadow-inner relative overflow-hidden ";
  if (status === 'error') containerClasses += "border-red-300 dark:border-red-500/50 hover:bg-red-50 dark:hover:bg-red-500/10";
  else if (status === 'success') containerClasses += "border-green-300 dark:border-green-500/50 bg-green-50 dark:bg-green-500/5 hover:bg-green-100 dark:hover:bg-green-500/10";
  else containerClasses += "border-gray-300 dark:border-[#2A2D33] hover:bg-gray-100 dark:hover:bg-[#1C1F24] hover:border-[#14F1E1]";
        ...
        {status === 'processing' ? (
          ...
          <>
            ...
            <div className="text-left">
              <span className="text-green-600 dark:text-green-400 font-bold text-sm block">Upload Complete</span>
              <span className="text-green-600/70 dark:text-green-300/70 text-[10px] block leading-none">{message}</span>
            </div>
          </>
        ) : status === 'error' ? (
          <>
            ...
            <div className="text-left">
              <span className="text-red-600 dark:text-red-400 font-bold text-sm block">Upload Failed</span>
              <span className="text-red-600/70 dark:text-red-300/70 text-[10px] block leading-tight max-w-[200px]">{message}</span>
            </div>
          </>
        ) : (
          <>
            <div className="bg-gray-200 dark:bg-[#1C1F24] p-1.5 rounded-full group-hover:scale-110 transition-transform shadow-lg group-hover:shadow-[0_0_10px_rgba(20,241,225,0.2)]">
              <FileUp className="w-4 h-4 text-[#5B6CFF] group-hover:text-[#14F1E1] transition-colors" />
            </div>
            <div className="text-left">
              <span className="text-gray-700 dark:text-gray-300 font-medium text-sm group-hover:text-gray-900 dark:group-hover:text-white transition-colors block">Upload Data File</span>
              <span className="text-gray-500 dark:text-gray-600 text-[10px] block leading-none">Auto-detects Columns</span>
            </div>
          </>
        )}
      </label>
